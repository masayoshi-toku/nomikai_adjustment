class ReactionsController < ApplicationController
  ...
  def create
    @reaction_form = ReactionForm.new(reaction_params.merge(user: current_user, event_url_path: params[:event_url_path]))
    if @reaction_form.create
      redirect_to event_url(@reaction_form.event_url_path), notice: '出席の登録に成功しました。'
    else
